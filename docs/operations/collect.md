...
拒み、その行を **`blocked`** にする。**諸元表からの取り込みは既存グレードの諸元を
更新できるが、新しいグレードは作れない。** 新規グレードの作成には価格の取得元
（設計書7.4の案B）か、管理画面での人手入力が要る。
...
**現在の状態:** プリウス（2026-07版）の諸元表には `new_grade` が8件と `discontinued` が
1件、同じ書類として承認キューにある。承認して適用を押すと、`new_grade` 8件は
`blocked` になって残り、`grades` は変わらない。これは異常ではなく、価格が無いという
事実が画面に出ている正常な状態である。
